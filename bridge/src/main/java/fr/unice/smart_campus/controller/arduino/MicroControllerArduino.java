...
throws ControllerException
{
   ...
   // Build sensor endpoint URL.
   tkz.nextToken();
   String sensorEndPointURL = tkz.nextToken();

   // Build sensor endpoint Port.
   tkz.nextToken();
   int sensorEndPointPort = Integer.parseInt(tkz.nextToken());
   // Return the descriptor.
   return new SensorDescriptor(sensorName, pinNumber, sensorFrequency, sensorEndPointURL, sensorEndPointPort);
}
...
throws NoSuchFieldException, SecurityException, IOException
{
   ...
   // Get sensor network end-point
   String endpoint = configuration.getSensorFromName(sd.getSensorName()).getEndPoint();
   
   // Send data to collector
   RestClient.sendToCollector(sd, endpoint);
